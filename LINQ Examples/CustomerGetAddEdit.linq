...
void Main()
{
	...
	codeBehind.AddEditCustomer(customer);
	//codeBehind.ErrorDetails.Dump("Duplicate customer");
	...
	//	City = "My City",
	//	ProvStateID = Lookups.Where(l => l.Name == "Alberta").Select(l => l.LookupID).FirstOrDefault(),
	//	//ProvStateID = Lookups.Where(l => l.Name == "Alberta").FirstOrDefault().LookupID,
	//	CountryID = Lookups.Where(l => l.Name == "Canada").Select(l => l.LookupID).FirstOrDefault(),
	//	Phone = "123456789",
	//	PostalCode = "T6W5R2",
	//	Email = "email@gamil.com",
	//	StatusID = Lookups.Where(l => l.Name == "Silver").Select(l => l.LookupID).FirstOrDefault(),
	//	RemoveFromViewFlag = false
	//};
	
	//Customers.OrderByDescending(c => c.CustomerID).Take(2).Dump("Before Adding");
	//codeBehind.AddEditCustomer(customer);
	//codeBehind.ErrorDetails.Dump();
	//codeBehind.Customer.Dump("New customer");
	//Customers.OrderByDescending(c => c.CustomerID).Take(3).Dump("After Adding");
	
	codeBehind.GetCustomer(1033);
	customer = codeBehind.Customer;
	customer.Address1 = "My Address 1";
	customer.Address2 = "My Address 2";
	codeBehind.AddEditCustomer(customer);
	codeBehind.ErrorDetails.Dump();
...
public class CodeBehind(TypedDataContext context)
{
	...
	public void AddEditCustomer(CustomerEditView customer)
	{
		...
		try
		{
			Result<CustomerEditView> result = YourService.AddEditCustomer(customer);
