<Query Kind="Program">
  <Connection>
    <ID>b07cb2bb-ef41-49b3-a66d-26a76f66d451</ID>
...
void Main()
{
	...
	phone = "999999";
	codeBehind.GetCustomers(lastName, phone);
	//codeBehind.ErrorDetails.Dump($"No customer with last name of {lastName} and phone number of {phone}");
...
public class CodeBehind(TypedDataContext context)
{
	...
	public void GetCustomers(string lastName, string phone)
	{
		...
		try
		{
			Result<List<CustomerSearchView>> result = YourService.GetCustomers(lastName, phone);
...
public class Library
{
	...
	public Result<List<CustomerSearchView>> GetCustomers(string lastName, string phone)
	{
		...
		Result<List<CustomerSearchView>> result = new();
		...
		var customers = _oltpDMIT2018Context.Customers
